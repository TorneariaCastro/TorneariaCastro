...
- ✅ ~~`proximoNumero()` gera o número da OS contando linhas~~ — **corrigido em 2026-09-10 (Fase 02.7)**. Substituído por contador no banco (`os_contador` + `public.proximo_numero_os()`, migration `0007`), incrementado numa única instrução atômica e consumido via `.rpc()`. Ver `docs/decisions/ADR-001-numeracao-de-ordens-de-servico.md`. **Em `dev`, ainda não promovido para produção.**
