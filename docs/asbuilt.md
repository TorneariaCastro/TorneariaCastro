...
**Status:** `⏳ Aguardando execução do Atlas` (spec da Shiva aprovada por Kleber em 2026-09-10; plano técnico do Hades pronto)
...
- [ ] 1. Migration `0006_itens_servico.sql` — tabela `itens_servico` + 4 políticas de RLS no padrão do `0002_roles.sql`
- [ ] 2. `ItemServico`, `calcularValorServicos` e `calcularValorTotal` somando os três blocos (`src/lib/types/ordem-servico.ts`)
- [ ] 3. `SELECT_ORDEM_SERVICO` + `toOrdemServico` trazendo os itens novos (`src/lib/data/ordens-servico.ts`)
- [ ] 4. Server Actions: `adicionarServico`, `removerItem` aceitando `"servico"`, e **`gerarContaAReceber` somando o bloco novo**
- [ ] 5. Bloco "Serviço" na UI, acima de Mão de obra (`itens-lancamentos.tsx`) + texto explicando que os números cinzas são exemplos
- [ ] 6. Página pública: para de buscar mão de obra e materiais do banco, mostra as linhas de Serviço, total calculado no servidor
- [ ] 7. Manual atualizado (bloco novo + campos-fantasma + vírgula) e **versionado no git** — hoje está untracked
...
**Testável:** OS com `Serviço R$ 800` + `Materiais R$ 120` → R$ 920 iguais na tela da OS, na listagem, na ficha do cliente, na página do cliente e na conta a receber gerada ao faturar.
