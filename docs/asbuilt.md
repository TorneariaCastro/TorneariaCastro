...
- 🟠 **`proximoNumero()` gera o número da OS contando linhas, não com uma sequence** (`src/app/(app)/ordens-servico/actions.ts:13-21`). Duas consequências reais: (1) apagar um subconjunto de OS faz o contador gerar um número já existente e a constraint `unique` derruba a criação da próxima OS com a mensagem genérica `"Não foi possível criar a ordem de serviço."`; (2) duas OS criadas no mesmo instante leem o mesmo `count` e colidem. Ironia registrada: o projeto **já usa o padrão correto** em outro lugar — `nfse_rps_sequencial` (migration `0003`) é uma sequence de verdade. Corrigir é uma migration + três linhas.
